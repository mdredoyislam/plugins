...
if ( !empty( $data['buttontext'] ) ) {
	$btn = '<a class="btn-style3" ' . $attr . '>'.'<span>' . $data['buttontext'] .$svg.'</span>'. '</a>';
}
$final_icon_class       = "";
...
$has_icon= $final_icon_class ? 'has-icon':'no-icon';
?>
<div class="cta-default cta-<?php echo esc_attr( $data['style'] ); ?>">
	<div class="action-box">
		<div class="cta-content <?php echo esc_attr($has_icon); ?>">
			<div class="content">
				<h2 class="cta-title"><?php echo wp_kses_post($data['title']); ?></h2>
				<?php if ( $btn ) { ?>
					<div class="cta-button "><?php echo wp_kses_post( $btn );?></div>		
				<?php } ?>
				<?php if($data['sub_title']){ ?>
					<h3 class="sub-title"><?php echo wp_kses_post($data['sub_title']); ?></h3>
				<?php } ?>
			</div>
