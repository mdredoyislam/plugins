...
$btn = '<a class="btn-style1" ' . $attr . '>'.'<span>' . $data['button_one'] .$svg.'</span>'.'</a>';
$section_img=wp_get_attachment_image($data['rt_image']['id'],'full');
?>
<div class="title-text-button  text-<?php echo esc_attr( $data['style'] ); ?>">
	<div class="section-bg">
		<?php if($section_img){
			echo wp_kses_post($section_img);
		} ?>
		<div class="title-text-wrapper">
			...
			<?php if ( !empty( $data['title'] ) ) { ?>
				<h2 class="section-title"><?php echo wp_kses_post( $data['title'] );?></h2><?php } ?>
			<div class="section-content"><?php echo wp_kses_post( $data['content'] );?></div>
			<?php if ( $data['button_display']  == 'yes' ) { ?>
				<?php if ( $btn ) { ?>
					<div class="section-button"><?php echo wp_kses_post( $btn );?></div>
				<?php } ?>
			<?php } ?>
		</div>
	</div>
