...
if( ! class_exists( 'RT_POST_VIEWS' )){
	...
	class RT_POST_VIEWS{
		...
		function _set_post_views(){
			...
			if( !GymatTheme::$options['post_view'] || ! is_single() ){
				return;
			}
			...
			if( !GymatTheme::$options['post_view'] || $page > 1  ){
				return false;
			}
		...
		function _posts_column_views( $defaults ){
			...
			if( GymatTheme::$options['post_view'] ){
				$defaults['post_view'] = esc_html__( 'Views', 'gymat-core' );
			}
...
/*----------------------------------------------------------*/
if( !function_exists( 'gymat_views' )){

	function gymat_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$gymat_view_html = esc_html__( 'View' , 'gymat-core' );
		} else {
			$gymat_view_html = esc_html__( 'Views' , 'gymat-core' );
		}
		
		$gymat_views_html = '<span class="view-number" >' . $view_count . '</span> ' . $gymat_view_html;

		return '<span class="meta-views meta-item '. $views_class .'">'. $gymat_views_html.'</span> ';
	}
}
