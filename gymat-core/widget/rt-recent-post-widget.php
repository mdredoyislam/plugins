...
* By : Radius Theme
*/
Class GymatTheme_Recent_Posts_With_Image_Widget extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			'classname' => 'rt_widget_recent_entries_with_image',
			'description' => esc_html__( 'Your site&#8217;s most recent Posts with Image.' , 'gymat-core' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'rt-recent-posts', esc_html__( 'Gymat : Recent Posts' , 'gymat-core' ), $widget_ops );
								...
								<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>" class="rt-wid-post-img">
									<?php
										if ( has_post_thumbnail() ){
											...
											?>
											<img class="rt-lazy" src="<?php echo esc_url( GYMAT_IMG_URL ); ?>noimage_100X98.jpg" alt="<?php the_title_attribute(); ?>">
									<?php
										}
									?>
								</a>
							...
							<div class="recent-post-content">
								<?php if ( $show_cat ) { ?>
									<div class="post-date1">
										<span><?php echo esc_html( gymat_get_primary_category()[0]->name ); ?></span>
									</div>
								<?php } ?>
								<?php if ( $show_date ) { ?>
